...
suite('Unit Tests', function () {
    suite('数字测试', function () {
        // #1
        test('正确读取整个输入', function (done) {
            assert.equal(convertHandler.getNum('2kg'), 2);
            done();
        });
        // #2
        test('正确读取一个带小数点输入', function (done) {
    ...
    suite('单位测试', function () {
        ...
        // #10
        test('正确返回每个有效输入单位的全称', function (done) {
            assert.equal(convertHandler.spellOutUnit('lbs'), 'pounds');
            done();
        })
        // #11
        test('正确地将 gal 转换为 L', function (done) {
            assert.equal(convertHandler.convert(1, 'gal'), 3.78541);
            done();
        });
        // #12
        test('正确地将 L 转换为 gal', function (done) {
            assert.equal(convertHandler.convert(1, 'L'), 0.26417);
            done();
        });
        // #13
        test('正确地将 mi 转换为 km', function (done) {
            assert.equal(convertHandler.convert(1, 'mi'), 1.60934);
            done();
        });
        // #14
        test('正确地将 km 转换为 mi', function (done) {
            assert.equal(convertHandler.convert(1, 'km'), 0.62137);
            done();
        });
        // #15
        test('正确地将 lbs 转换为 kg', function (done) {
            assert.equal(convertHandler.convert(1, 'lbs'), 0.45359);
            done();
        });
        // #16
        test('正确地将 kg 转换为 lbs', function (done) {
